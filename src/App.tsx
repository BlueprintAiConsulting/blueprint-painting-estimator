...
import { countWhitePixels } from './utils/pixelMeasurement';
...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const vizPromise = fetch(`${API_BASE}/api/paint-visualize`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          imageBase64: base64,
  ...
  return (
    <div className="min-h-screen bg-[#060B18] text-[#E2E8F0] font-sans antialiased overflow-x-hidden">
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 py-4 sm:py-6">
        <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 pb-28 lg:pb-0">
          {/* LEFT PANEL — Controls */}
          <div className="w-full lg:w-4/12 flex flex-col gap-4 lg:gap-5 order-2 lg:order-1">
            
            {/* Top Controls (Above Canvas on Mobile) */}
            <div className="order-first lg:order-none space-y-4 lg:space-y-5">
              {/* Room Type */}
            ...
            {/* Middle Controls (Below Canvas on Mobile) */}
            <div className="order-3 lg:order-none space-y-4 lg:space-y-5">
              {/* Paint Color Selection */}
              <div className="rounded-xl border border-[#1E293B] overflow-hidden">
                <div className="px-4 sm:px-5 py-3 sm:py-3.5 bg-[#111827] border-b border-[#1E293B] flex items-center gap-3">
              ...
              {showEstimator && resultImage && (
                <PaintEstimator 
                  ...
                  exactWallPixels={exactWallPixels || undefined}
                  pixelsPerFoot={pixelsPerFoot || undefined}
                />
              )}
            </div>

            {/* Bottom Floating Action Bar (Mobile Sticky) */}
            <div className="order-4 lg:order-none fixed bottom-0 left-0 right-0 p-4 bg-[#060B18]/95 backdrop-blur-xl border-t border-[#1E293B] z-50 lg:hidden">
              <div className="flex gap-2 max-w-[1600px] mx-auto">
                {resultImage && (
                  <button
                    onClick={() => { setResultImage(null); setShowEstimator(false); }}
                    className="w-[100px] sm:w-[120px] py-4 rounded-lg font-bold text-[#94A3B8] bg-[#1E293B] hover:bg-[#334155] hover:text-white transition-all text-[10px] tracking-widest uppercase border border-[#334155] flex flex-col items-center justify-center gap-1"
                  >← Edit</button>
                )}
                <button
                  disabled={ai.isQuickGenerating || !selectedImage || !zones.some(z => z.enabled)}
                  onClick={handleGenerate}
                  className={`flex-1 py-4 rounded-lg font-bold text-white shadow-lg flex items-center justify-center gap-3 transition-all uppercase tracking-wider text-[11px] min-h-[44px] ${
          ...
          {/* RIGHT PANEL — Canvas */}
          <div className="w-full lg:w-8/12 order-1 lg:order-2 sticky top-14 lg:top-4 z-40 lg:z-auto self-start flex flex-col gap-4">
            <div className={`bg-[#111827] rounded-xl border border-[#1E293B] p-1 flex flex-col shadow-2xl overflow-hidden transition-all duration-300 ${!selectedImage ? 'h-[25vh] sm:h-[30vh]' : 'h-[45vh] sm:h-[50vh]'} lg:h-[calc(100vh-140px)] lg:min-h-[400px]`}>
              <div className="bg-[#0F172A] border-b border-[#1E293B] px-4 py-3 flex items-center justify-between shrink-0">
              ...
              <VisualizerCanvas
                ...
                setPixelsPerFoot={setPixelsPerFoot}
              />
            </div>

            {/* Desktop Generate CTA (Hidden on mobile) */}
            <div className="hidden lg:flex gap-3">
              {resultImage && (
                <button
                  onClick={() => { setResultImage(null); setShowEstimator(false); }}
                  className="w-[140px] py-4 rounded-lg font-bold text-[#94A3B8] bg-[#1E293B] hover:bg-[#334155] hover:text-white transition-all text-[11px] tracking-widest uppercase border border-[#334155] flex items-center justify-center gap-2 shadow-lg"
                >← Edit Photo</button>
              )}
              <button
                disabled={ai.isQuickGenerating || !selectedImage || !zones.some(z => z.enabled)}
                onClick={handleGenerate}
                className={`flex-1 py-4 rounded-lg font-bold text-white shadow-lg flex items-center justify-center gap-3 transition-all uppercase tracking-wider text-[12px] min-h-[52px] ${
                  ai.isQuickGenerating || !selectedImage || !zones.some(z => z.enabled)
                    ? 'bg-[#1E293B] text-[#64748B] cursor-not-allowed border border-[#334155]'
                    : resultImage
                      ? 'bg-[#10B981] hover:bg-[#059669] text-white shadow-[0_0_30px_rgba(16,185,129,0.3)] hover:shadow-[0_0_40px_rgba(16,185,129,0.5)] hover:-translate-y-0.5'
                      : 'bg-[#3B82F6] hover:bg-[#2563EB] text-white shadow-[0_0_30px_rgba(59,130,246,0.3)] hover:shadow-[0_0_40px_rgba(59,130,246,0.5)] hover:-translate-y-0.5'
                }`}
              >
                {ai.isQuickGenerating
                  ? <><Loader2 className="w-5 h-5 animate-spin" /> Painting Room...</>
                  : resultImage
                    ? <><Sparkles className="w-5 h-5" /> Re-Visualize Colors</>
                    : <><Paintbrush className="w-5 h-5" /> Preview My Paint Colors</>}
              </button>
            </div>
          </div>
        </div>
